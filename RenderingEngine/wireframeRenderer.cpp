...
Mesh* WireframeRenderer::selectWireframeMesh(BoundVolumeEnum typeOfBound) {
	switch (typeOfBound) {
...
void WireframeRenderer::draw(const Graphics& graphics, const std::vector<std::pair<Object*, Mesh*>>& objQ) {
...
void WireframeRenderer::drawInstanced(Graphics* graphics, const std::vector<std::pair<Object*, Mesh*>>& objQ) {
	if (objQ.size() == 0 || !graphics->setting.showBoundingVolume) return;
